...
export class ReportIncomeComponent implements OnInit, OnDestroy {
    ...
    export() {
        this.loading = true
        this.exportsSubscription = this.reportService.reportMemberIncome(this.date.value.startDate!, this.date.value.endDate!).pipe(finalize(() => this.loading = false)).subscribe(result => {
