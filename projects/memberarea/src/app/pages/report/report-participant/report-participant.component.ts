...
export class ReportMemberComponent implements OnInit, OnDestroy {
    ...
    init() {
        this.reportsSubscription = this.reportService.getAllActivityMember(this.date.value.startDate!, this.date.value.endDate!, this.first, this.limit).subscribe(result => {
            this.reportsRes = result.data
        })
        this.countSubscription = this.reportService.countActivityMember(this.date.value.startDate!, this.date.value.endDate!).subscribe(result => {
    ...
    getData(offset: number, limit: number) {
        this.pageChangeSubscription = this.reportService.getAllActivityMember(this.date.value.startDate!, this.date.value.endDate!, offset, limit).subscribe(result => {
            this.reportsRes = result.data
        })
        this.countSubscription = this.reportService.countActivityMember(this.date.value.startDate!, this.date.value.endDate!).subscribe(result => {
            this.totalReports = result
